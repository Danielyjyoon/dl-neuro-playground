...
from load_neuroimaging_data import OrigDataThickSlices
from load_neuroimaging_data import map_label2aparc_aseg
from load_neuroimaging_data import map_prediction_sagittal2full
from load_neuroimaging_data import get_largest_cc
from load_neuroimaging_data import load_and_conform_image
from load_neuroimaging_data import save_image

from augmentation import ToTensorTest_CT

from networks import FastSurferCNN

def options_parse(args, parc_home):
    ...
    parser.add_argument('--num_filters', type=int, default=256,
                        help='Filter dimensions for DenseNet (all layers same). Default=128')
    ...
    parser.add_argument('--batch_size', type=int, default=args.batch_CT_Parc, help="Batch size for inference. Default: 8")

    sel_option = parser.parse_args()

    return sel_option

def run_network(img_filename, orig_data, prediction_probability, plane, ckpts, params_model, model, logger, small_gpu):

    # Set up DataLoader
    test_dataset = OrigDataThickSlices(img_filename, orig_data, plane=plane,
                                       transforms=transforms.Compose([ToTensorTest_CT()]))
...
def CT_parcellation(args, parc_home, file_ST, file_ST_256, file_ST_seg, ST_nifti, parc_results, logger):
    ...
    try:
        ...
        options = options_parse(args, parc_home)
        ...
        header_info, affine_info, orig_data = load_and_conform_image(file_ST, ST_nifti, 1, logger)

        # Save conformed CT input image
        parc_results['ST']= save_image(orig_data, affine_info, header_info, file_ST_256, args.save_files)
        
        logger.info("Saving conformed original image to {}".format(file_ST_256))
        ...
        if not args.small_gpu:
            pred_prob = torch.zeros((256, 256, 256, options.num_classes_ax_cor), dtype=torch.float, device=device)
        else:
            pred_prob = torch.zeros((256, 256, 256, options.num_classes_ax_cor), dtype=torch.float)
        ...
        pred_prob = run_network(file_ST,
                                ...
                                params_model, model, logger, args.small_gpu)
        ...
        parc_results['LABEL']= save_image(pred_prob, affine_info, header_info, file_ST_seg, args.save_files)

        logger.info("Saving Segmentation to {}".format(file_ST_seg))
        logger.info("Parcellation time total: {:0.4f} seconds.".format(time.time() - start_total))
        torch.cuda.empty_cache()

    except Exception as e:
        torch.cuda.empty_cache()
        error_code = 0x6000
        # status.error.set(error_code)
        logger.exception(e)
        logger.error(f"failed: {hex(error_code)}", 'e', 'CT Parcellation')